...
export default function SongGrid({ songs, playlists }: Props) {
  ...
  if (songs.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)', background: 'rgba(243,186,47,0.01)', border: '1px dashed var(--glass-border)', borderRadius: '16px' }}>
      ...
      {songs.map((song) => (
        <div key={song.id} className="song-card" style={{ position: 'relative' }}>
          <div
            ...
            style={{
              width: '100%', aspectRatio: '1', background: '#1a1a1a', borderRadius: '8px',
              marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '1.5rem', position: 'relative', cursor: 'pointer', overflow: 'hidden',
              ...
              {showMenu === song.id && (
                <div className="glass" style={{ position: 'absolute', bottom: '100%', right: 0, width: '180px', zIndex: 100, padding: '8px', marginBottom: '8px' }}>
                  ...
                  <div style={{ maxHeight: '120px', overflowY: 'auto', marginTop: '4px' }}>
                    {playlists.map(p => (
                      <div key={p.id} onClick={() => addToPlaylist(song.id, p.id)}
